class RefurbishedSmartphones {
    ...
    sellSmartphone(model, desiredStorate) {
        ...
        if (!foundPhone) {
            ...
            if (foundPhone.storage >= desiredStorate) {
                soldPrice = foundPhone.price;
            } else if (Math.abs(foundPhone.storage - desiredStorate) <= 128) { 
                soldPrice = foundPhone.price - (foundPhone.price * 0.1);
            } else if (Math.abs(foundPhone.storage - desiredStorate) > 128) { 
                soldPrice = foundPhone.price - (foundPhone.price * 0.2);
            }

            this.soldSmartphones.push({ model: foundPhone.model, storage: foundPhone.storage, soldPrice });
    ...
    upgradePhones() {
        ...
        this.availableSmartphones = this.availableSmartphones.map((phone) => {
            ...
            const doubleStorage = phone.storage * 2;
        ...
        const availablePhones = this.availableSmartphones.map((phone) => `${phone.model} / ${phone.storage} GB / ${phone.condition} condition / ${phone.price}$`).join('\n');
